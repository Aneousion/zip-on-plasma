...
function TutorialOverlay({ step }: TutorialOverlayProps) {
    ...
    const findTarget = () => {
      ...
      if (!element) {
        if (step.id === 'tab-navigation') {
          element = document.querySelector('button[class*="text-[#5fb574]"]') ||
                   document.querySelector('main button:first-of-type');
        } else if (step.id === 'time-filters') {
          element = document.querySelector('button[class*="bg-[#5fb574]"]') ||
  ...
  }, [step.target, step.id]); // Remove targetElement from dependencies
